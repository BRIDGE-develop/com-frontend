...
const Paycheck: React.FC = (): JSX.Element => {
    const userName = '李振燮';
    const items = [
    ...
    const clickChangeMonth = (
        e: React.MouseEvent<HTMLAnchorElement>,
        data: MenuItemProps
    ): void => {
        setValues({ date: data.name as string, salary: data.salary, addMode: false });
    };

    const clickChangeMode = (): void => {
        setValues({ date: '' as string, salary: 0, addMode: true });
    };

    const changeDropdown = (e: React.SyntheticEvent<HTMLElement>, data: DropdownProps): void => {
